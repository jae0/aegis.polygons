...
areal_units = function( p=NULL, areal_units_fn_full=NULL, plotit=FALSE, sa_threshold_km2=0, redo=FALSE, use_stmv_solution=FALSE, rastermethod="sf",  xydata=NULL, constraintdata=NULL, spbuffer=5, hull_multiplier = 6, ... ) {
  ...
  if (p$areal_units_type %in% c( "stratanal_polygons_pre2014", "stratanal_polygons", "groundfish_strata")  ) {
  ...
  if (p$areal_units_type %in% c( "stratanal_polygons_post2014")  ) {
    ## using the "standard" polygon definitions  .. see https://cran.r-project.org/web/packages/spdep/vignettes/nb.pdf
    # Here we compute surface area of each polygon via projection to utm or some other appropriate planar projection.
    # This adds some variabilty relative to "statanal" (which uses sa in sq nautical miles, btw)
  ...
  if (!is.null(boundary)) {
    ...
    if ( p$areal_units_type == "inla_mesh" ) {
    ...
    if ( p$areal_units_type == "tesselation" ) {
      sppoly = aegis.polygons::aegis_mesh( 
        pts=xydata, 
        boundary=boundary, 
        resolution=p$areal_units_resolution_km, 
        spbuffer=p$areal_units_resolution_km, 
        areal_units_constraint_nmin=p$areal_units_constraint_nmin, 
        tus=tus,
  ...
  # Overlays
    sppoly = areal_units_overlay(
      sppoly = sppoly, 
      areal_units_overlay = p$areal_units_overlay, 
      areal_units_resolution_km = p$areal_units_resolution_km, 
      areal_units_proj4string_planar_km = p$areal_units_proj4string_planar_km, 
      aegis_internal_resolution_km = p$aegis_internal_resolution_km ,
  ...
  if (p$areal_units_constraint == "groundfish")  constraintdata = survey_db( DS="set.base", p=p )[, c("lon", "lat")]  #
  if (p$areal_units_constraint == "snowcrab")    constraintdata = snowcrab.db( p=p, DS="set.clean" )[, c("lon", "lat")]  #
  ...
  # ... but they require SA estimates after any filters (above))
    if ( grepl("snowcrab_managementareas", p$areal_units_overlay) ) {
      ...
      for (subarea in c("cfanorth", "cfasouth", "cfa23", "cfa24", "cfa4x" ) ) {
        ...
        csa = st_transform( csa, st_crs( p$areal_units_proj4string_planar_km ) )
